...
class OrderFilterForUserView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Order.objects.filter(creator=self.request.user , status=filter_method).order_by('-date_created')
...
class OrderFilterForStoreView(ListAPIView):
    ...
    def get_queryset(self):
        filter_method = self.request.query_params.get('filter_method')
        store = Store.objects.get(owner=self.request.user)
        return Order.objects.filter(store=store , status=filter_method).order_by('-date_created')
